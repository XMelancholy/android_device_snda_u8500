			 ...
			 const u8 *req_dev_types, const u8 *dev_id)
{
	...
	params.rssi_filter_value = wpa_s->rssi_filter_value;
#endif
					   ...
					   int success)
{
	...
	if (client) {
		...
		wpa_msg(wpa_s->parent, MSG_INFO, P2P_EVENT_GROUP_STARTED
			...
			ssid->frequency > 0 ? ssid->frequency : wpa_s->current_bss->freq,
			psk,
			MAC2STR(go_dev_addr),
		...
		wpa_msg(wpa_s->parent, MSG_INFO, P2P_EVENT_GROUP_STARTED
			...
			wpa_s->ifname, ssid_txt,
			ssid && ssid->frequency > 0 ? ssid->frequency : wpa_s->current_bss->freq,
...
#ifdef ENABLE_STE_CHANGES
	/*
	* P2P GO based on BCM chip will reject association
	* requests containing P2P IE if the STA is trying
	* to connect as legacy (non P2P) station.
	* As a workaround do not add P2P IE in association
	* request if connecting in legacy mode and vendor
	* specific BCM IE present in GO beacons/probe resp.
	*/
	bcm_ie = wpa_bss_get_vendor_ie(bss, BCM_IE_VENDOR_TYPE);
...
void wpas_p2p_completed(struct wpa_supplicant *wpa_s)
{
	...
	if (ssid->passphrase == NULL && ssid->psk_set) {
		...
		wpa_msg(wpa_s->parent, MSG_INFO, P2P_EVENT_GROUP_STARTED
			...
			persistent ? " [PERSISTENT]" : "");
#ifdef ENABLE_STE_CHANGES
		ssid->p2p_mode = P2P_CLIENT;
		switch (wpa_s->assoc_freq) {
			case 2412:
			case 2437:
			case 2462:
				(void) p2p_update_listen_channel(
						wpa_s->global->p2p,
						(wpa_s->assoc_freq - 2407) / 5);
				break;
			default:
				break;
		}
		     ...
		     int duration, int interval)
{
	...
	struct noa_attr *attr = &wpa_s->noa_attr;
	...
	ret = hostapd_p2p_set_noa(wpa_s->ap_iface->bss[0], count, start,
				  duration, interval);

	return ret;
}

int wpas_set_p2p_powersave(struct wpa_supplicant *wpa_s, int legacy_ps,
			   int opp_ps, int ctwindow)
{
	...
	return hostapd_set_p2p_powersave(hapd, legacy_ps, opp_ps, ctwindow);
}
...
#ifdef ENABLE_STE_CHANGES
/**
 * wpas_handle_notify_noa - Handle driver NOA notification
 *
 */

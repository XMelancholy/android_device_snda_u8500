...
#ifdef CONFIG_WPS_ER
static int wpa_supplicant_ctrl_iface_wps_reg_answer(
				struct wpa_supplicant *wpa_s, char *cmd)
{
	if(!cmd)
		return -1;
	return wpas_wps_start_reg_answer(wpa_s, cmd);
}
#endif /* CONFIG_WPS_ER */
#endif /* ENABLE_STE_CHANGES */
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (ssid == wpa_s->current_ssid) {
		wpa_sm_set_config(wpa_s->wpa, NULL);
		eapol_sm_notify_config(wpa_s->eapol, NULL, NULL);
		/* Set the Current ssid to NULL as the ssid data has already been cleared above*/
...
static int p2p_ctrl_set(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*param++ = '\0';
	...
	if (os_strcmp(cmd, "noa") == 0) {
		...
		/* GO NoA parameters: count,start_offset(us),duration(us),interval (us) */
		count = atoi(param);
		pos = os_strchr(param, ',');
		if (pos == NULL)
			return -1;
		...
		pos++;
		interval = atoi(pos);
		if (count < 0 || count > 255 || start < 0 || duration < 0 || interval < 0)
			return -1;
		if (count == 0 && duration > 0)
			return -1;
		if (9 * interval < 10 * duration)
			return -1;
		if (duration > 800000)
			return -1;
	...
	if (os_strcmp(cmd, "oppps") == 0) {
		...
		int opp_ps, ctwindow;
		int ret;
		/* GO OPP_PS parameters: opp_ps,ctwindow */
		opp_ps = atoi(param);
		pos = os_strchr(param, ',');
		if (pos == NULL)
			return wpa_drv_set_p2p_powersave(wpa_s, -1, opp_ps, -1);
		pos++;
		ctwindow = atoi(pos);
		if (opp_ps < 0 || ctwindow < 0 || ctwindow > 127)
			return -1;
		return wpa_drv_set_p2p_powersave(wpa_s, -1, opp_ps, ctwindow);
	}
#else
	if (os_strcmp(cmd, "oppps") == 0)
...
#ifdef ENABLE_STE_CHANGES
	if ((reply_len = wpa_supplicant_ctrl_iface_process_ste_commands(wpa_s, buf, reply, reply_size)) != 0)
	{
	  *resp_len = reply_len;
	  return reply;
	}
...
#ifdef ENABLE_STE_CHANGES
#ifdef CONFIG_WPS_ER
	} else if (os_strncmp(buf, "WPS_REG_ANSWER ", 15) == 0) {
		if (wpa_supplicant_ctrl_iface_wps_reg_answer(wpa_s, buf + 15))
			reply_len = -1;
#endif /* CONFIG_WPS_ER */
#endif /* ENABLE_STE_CHANGES */
#ifdef CONFIG_AP

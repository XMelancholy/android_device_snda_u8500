				   ...
				   const char *params)
{
	...
	pos = os_strchr(pos, ':');
	if(pos == NULL) {
		data->beacon_threshold = 20;
		data->tx_threshold = 0;
		return 0;
	}
	pos++;
	data->beacon_threshold = atoi(pos);
	pos = os_strchr(pos, ':');
	if(pos == NULL) {
		data->tx_threshold = 0;
		return 0;
...
#ifdef ENABLE_STE_CHANGES
#ifdef CONFIG_P2P
	/* Disable roaming/bgscan when P2P_CLIENT */
	if (ssid->p2p_mode == P2P_CLIENT) {
		wpa_printf(MSG_DEBUG, "bgscan ste: Disabling roaming (P2P_CLIENT)\n");
		return NULL;
	}
#endif /*CONFIG_P2P */
#endif /* ENABLE_STE_CHANGES */
...
#ifdef ENABLE_STE_CHANGES
		wpa_drv_signal_monitor_rssi(wpa_s, data->signal_threshold, 4) < 0) {
#else
...
#ifdef ENABLE_STE_CHANGES
	if(data->beacon_threshold &&
	   wpa_drv_signal_monitor_beacon_miss(wpa_s, data->beacon_threshold) < 0) {
		wpa_printf(MSG_ERROR, "bgscan ste: Failed to enable "
			   "beacon loss monitoring");
	}

	if(data->tx_threshold &&
	   wpa_drv_signal_monitor_tx_fail(wpa_s, data->tx_threshold) < 0) {
		wpa_printf(MSG_ERROR, "bgscan ste: Failed to enable "
...
#ifdef ENABLE_STE_CHANGES
	if (data->signal_threshold)
                wpa_drv_signal_monitor_rssi(data->wpa_s, 0, 0);
        if (data->beacon_threshold)
                wpa_drv_signal_monitor_beacon_miss(data->wpa_s, 0);
        if (data->tx_threshold)
                wpa_drv_signal_monitor_tx_fail(data->wpa_s, 0);
#else
        if (data->signal_threshold)
...
static void bgscan_learn_notify_beacon_loss(void *priv)
{
	...
	if(data->beacon_threshold &&
	   wpa_drv_signal_monitor_beacon_miss(data->wpa_s, data->beacon_threshold) < 0) {
		wpa_printf(MSG_ERROR, "bgscan learn: Failed to enable "
			   "beacon loss monitoring from beacon_loss");
	}
#else
        wpa_printf(MSG_DEBUG, "bgscan learn: beacon loss");
...
#ifdef ENABLE_STE_CHANGES
static void bgscan_learn_ste_notify_tx_failure(void *priv)
{
	struct bgscan_learn_data *data = priv;

	wpa_printf(MSG_DEBUG, "bgscan learn: tx failure, "
		   "trigger immediate scan");
	data->quick_scan = 1;
	eloop_cancel_timeout(bgscan_learn_timeout, data, NULL);
	eloop_register_timeout(0, 0, bgscan_learn_timeout, data,
	...
	if(data->tx_threshold &&
	   wpa_drv_signal_monitor_tx_fail(data->wpa_s, data->tx_threshold) < 0) {
		wpa_printf(MSG_ERROR, "bgscan learn: Failed to enable "
			   "tx failure monitoring from tx_failure");
	}
}
#endif
...
#ifdef ENABLE_STE_CHANGES
static int bgscan_learn_check_quick_scan(void *priv)
{
	struct bgscan_learn_data *data = priv;
	return data->quick_scan;
}

static void bgscan_learn_set_quick_scan(void *priv, int quick_scan)
{
	struct bgscan_learn_data *data = priv;
	data->quick_scan = quick_scan;
}
#endif /* ENABLE_STE_CHANGES */
...
#ifdef ENABLE_STE_CHANGES
	.notify_tx_failure = bgscan_learn_ste_notify_tx_failure,
	.check_quick_scan = bgscan_learn_check_quick_scan,
	.set_quick_scan = bgscan_learn_set_quick_scan,
#endif /* ENABLE_STE_CHANGES */

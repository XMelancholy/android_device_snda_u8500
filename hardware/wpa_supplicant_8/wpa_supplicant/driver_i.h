...
static inline int wpa_drv_signal_monitor_rssi(struct wpa_supplicant *wpa_s,
					 int threshold, int hysteresis)
{
	if (wpa_s->driver->signal_monitor_rssi)
		return wpa_s->driver->signal_monitor_rssi(wpa_s->drv_priv,
						     threshold, hysteresis);
	return -1;
}

static inline int wpa_drv_signal_monitor_beacon_miss(struct wpa_supplicant *wpa_s,
						     int beacon_threshold)
{
	if(wpa_s->driver->signal_monitor_beacon_miss)
		return wpa_s->driver->signal_monitor_beacon_miss(
					wpa_s->drv_priv, beacon_threshold);
	return -1;
}

static inline int wpa_drv_signal_monitor_tx_fail(struct wpa_supplicant *wpa_s,
						 int tx_threshold)
{
	if(wpa_s->driver->signal_monitor_tx_fail)
		return wpa_s->driver->signal_monitor_tx_fail(
					wpa_s->drv_priv, tx_threshold);
	return -1;
}
...
#endif
static inline int wpa_drv_get_sta(struct wpa_supplicant *wpa_s, struct hostap_sta_driver_data *data)
{
	if (wpa_s->driver->read_sta_data)
		return wpa_s->driver->read_sta_data(wpa_s->drv_priv, data, wpa_s->bssid);
	return -ENOSYS;
}
...
static inline int wpa_drv_set_data_filter(struct wpa_supplicant *wpa_s,
					const u8 *data, size_t data_len)
{
	if (wpa_s->driver->set_data_filter) {
		return wpa_s->driver->set_data_filter(wpa_s->drv_priv,
					data, data_len);
	}
	return -1;
}

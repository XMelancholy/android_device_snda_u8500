			...
			struct wpa_scan_res *res)
{
	...
	bss->ssid_len = ssid_len;
	bss->last_association.sec = 0;
	bss->last_association.usec = 0;
...
const u8 * wpa_bss_get_ie(const struct wpa_bss *bss, u8 ie)
{
	...
	if((bss->caps & IEEE80211_CAP_PRIVACY) && (ie == WLAN_EID_RSN))
		end = pos + bss->ie_len + bss->beacon_ie_len;
	else
...
#ifdef ENABLE_STE_CHANGES
	if((bss->caps & IEEE80211_CAP_PRIVACY) && (vendor_type == WPA_IE_VENDOR_TYPE))
		end = pos + bss->ie_len + bss->beacon_ie_len;
	else
#endif /* ENABLE_STE_CHANGES */
		end = pos + bss->ie_len;

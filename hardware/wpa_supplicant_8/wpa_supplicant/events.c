...
static void wpa_supplicant_stop_assoc_penalty(void *eloop_ctx,
                                               void *sock_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;

	if (wpa_s->assoc_penalty.penalty) {
		wpa_s->assoc_penalty.penalty = 0;
		os_memset(wpa_s->assoc_penalty.bssid, 0, ETH_ALEN);
		wpa_printf(MSG_DEBUG, "WPA: Assoc penalty stopped");
	}
}
...
#ifdef ENABLE_STE_CHANGES
		if (e->count > limit || wpa_s->countermeasures) {
#else
		if (e->count > limit) {
#endif /* ENABLE_STE_CHANGES */
...
#ifdef ENABLE_STE_CHANGES
		if (wpa_s->assoc_penalty.penalty &&
		    !os_memcmp(bss->bssid, wpa_s->assoc_penalty.bssid, ETH_ALEN)) {
			wpa_dbg(wpa_s, MSG_DEBUG, "   skip - beacon missed, "
				"assoc penalty");
			continue;
		}
#endif /* ENABLE_STE_CHANGES */
				       ...
				       struct wpa_scan_results *scan_res)
{
...
#ifdef ENABLE_STE_CHANGES
	/* Do not roam outside ESS if we have hit beacon miss/tx fail.
	   We can still regain before connection loss */
	if (wpa_s->current_ssid != ssid && wpa_s->assoc_penalty.penalty)
		return 0;
	else if (wpa_s->current_ssid != ssid)
...
#ifdef ENABLE_STE_CHANGES
	/* Force roaming immediately if bgscan has noticed beacon miss or TX-fail */
	if (wpa_s->bgscan) {
		for (i = 0; i < ETH_ALEN; i++) {
			if (wpa_s->assoc_penalty.bssid[i] != 0) {
				wpa_dbg(wpa_s, MSG_DEBUG, "Force roaming due to beacon missed");
				return 1;
			}
		}
	}
	...
	temporary_offset = 0;
	if(!os_get_time(&current_time) &&
		((current_time.sec - selected->last_association.sec) <
			BGSCAN_PENALTY_TIME)) {
				temporary_offset = BGSCAN_TEMPORARY_OFFSET;
	}

       min_diff = 2 + temporary_offset;

	if (current_bss->level < 0) {
		if (current_bss->level < -85)
			min_diff = 1 + temporary_offset;
		else if (current_bss->level < -80)
			min_diff = 2 + temporary_offset;
		else if (current_bss->level < -75)
			min_diff = 3 + temporary_offset;
		else if (current_bss->level < -70)
			min_diff = 4 + temporary_offset;
		else
			min_diff = 5 + temporary_offset;
	}
	if (abs(current_bss->level - selected->level) < min_diff) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Skip roam - too small difference "
			"signal level. Temporary offset(%i)", temporary_offset);
		return 0;
	}
...
static void update_last_associated(struct wpa_supplicant *wpa_s)
{
	struct wpa_bss *bss = NULL;

	if (wpa_s->current_ssid == NULL)
		return;
	bss = wpa_bss_get(wpa_s, wpa_s->bssid, wpa_s->current_ssid->ssid,
			  wpa_s->current_ssid->ssid_len);
	if(bss) {
		os_get_time(&bss->last_association);
	}
}
...
#endif
{
...
#ifdef ENABLE_STE_CHANGES
	if (bgscan_check_quick_scan(wpa_s)) {
		bgscan_set_quick_scan(wpa_s, 0);
		if (!selected ||
		    os_memcmp(wpa_s->current_bss->bssid,
		    selected->bssid, ETH_ALEN) == 0) {
			wpa_dbg(wpa_s, MSG_DEBUG, "bgscan: No suitable quick scan results "
				"- triggering full scan");
			wpa_scan_results_free(scan_res);
	...
	if (selected) {
		...
		(void)update_last_associated(wpa_s);
...
#ifndef CONFIG_NO_STDOUT_DEBUG
{
	...
	case EVENT_SIGNAL_CHANGE:
		switch(data->signal_change.event_mask)
		{
			case EVENT_SIGNAL_CHANGE_RSSI:
				bgscan_notify_signal_change(
					wpa_s, data->signal_change.above_threshold,
					data->signal_change.current_signal,
					data->signal_change.current_noise,
					data->signal_change.current_txrate);
				if (data->signal_change.above_threshold == NL80211_CQM_RSSI_THRESHOLD_EVENT_LOW){
					wpa_msg(wpa_s, MSG_INFO, "Connection quality monitor"
						" event: RSSI low");
				}
				else if (data->signal_change.above_threshold == NL80211_CQM_RSSI_THRESHOLD_EVENT_HIGH){
					wpa_msg(wpa_s, MSG_INFO, "Connection quality monitor"
						" event: RSSI high");
				}
				break;

			case EVENT_SIGNAL_CHANGE_BEACON:
				os_memcpy(wpa_s->assoc_penalty.bssid, wpa_s->bssid, ETH_ALEN);
				wpa_s->assoc_penalty.penalty = 1;

				eloop_register_timeout(10, 0,
				       wpa_supplicant_stop_assoc_penalty, wpa_s, NULL);

				bgscan_notify_beacon_loss(wpa_s);
				break;

			case EVENT_SIGNAL_CHANGE_TX:
				os_memcpy(wpa_s->assoc_penalty.bssid, wpa_s->bssid, ETH_ALEN);
				wpa_s->assoc_penalty.penalty = 1;

				eloop_register_timeout(10, 0,
				       wpa_supplicant_stop_assoc_penalty, wpa_s, NULL);
				bgscan_notify_tx_failure(wpa_s);
				break;
...
#endif /* CONFIG_P2P */
		break;
	case EVENT_DRIVER_NOTIFY_HANGED:

...
#ifdef CONFIG_WPS_ER
int wpas_wps_start_reg_answer(struct wpa_supplicant *wpa_s,
			      const char const *cmd)
{
	if(wpa_s && cmd) {
		eloop_cancel_timeout(wpas_wps_start_reg_cb, wpa_s, NULL);
		if(wpa_s->start_reg_ssid){
			if(os_strcmp(cmd, "yes") == 0) {

				wpa_msg(wpa_s, MSG_INFO, "WPS-REG: open network accepted.\n"
						  "Setting network authentication: OPEN");
				eloop_register_timeout(WPS_PBC_WALK_TIME, 0, wpas_wps_timeout,
			       				wpa_s, NULL);
				wpas_wps_reassoc(wpa_s, wpa_s->start_reg_ssid, 
							wpa_s->start_reg_ssid->bssid);
				wpa_s->start_reg_ssid = NULL;
				return 0;

			} else if(os_strcmp(cmd, "no") == 0) {
				wpas_clear_wps(wpa_s);
				wpa_s->start_reg_ssid = NULL;
				return 0;
				...
				wpas_clear_wps(wpa_s);
				wpa_s->start_reg_ssid = NULL;
				wpa_msg(wpa_s, MSG_INFO, "WPS-REG-ANSWER: Incorect answer "
							"Rerun start reg command. ");
				return -1;
			}
		}
		else{
			wpa_msg(wpa_s, MSG_INFO, "WPS-REG-ANSWER: wps_reg"
					" command timed out. Rerun start reg command. ");
			return -1;
		}
	}
	return -1;
...
void wpas_wps_start_reg_cb(void *eloop_data, void *user_ctx)
{
	struct wpa_supplicant *wpa_s = (struct wpa_supplicant*)eloop_data;
	if(wpa_s) {
		wpa_msg(wpa_s, MSG_INFO, "WPS-REG-TIMEOUT: No confirmation for OPEN network. "
				"\nOpen network canceled.");
		wpa_s->start_reg_ssid = NULL;
		wpas_clear_wps(wpa_s);
	}
}
...
#ifdef ENABLE_STE_CHANGES
#ifdef CONFIG_WPS_ER
	if (settings && (os_strcmp(settings->auth,"OPEN") == 0)) {
		eloop_cancel_timeout(wpas_wps_start_reg_cb, wpa_s, NULL);
		wpa_s->start_reg_ssid = ssid;
		res = eloop_register_timeout(WPS_CMD_CONFIRM_TIMEOUT, 0,
					     wpas_wps_start_reg_cb, wpa_s, NULL);
		if(res == 0){
			wpa_msg(wpa_s, MSG_INFO, "WPS-REG: Setting network authentication: OPEN. "
						"Are you sure? \nUse: wps_reg_answer yes/no");
			return 0;
			...
			wpa_s->start_reg_ssid = NULL;
			wpas_clear_wps(wpa_s);
			wpa_msg(wpa_s, MSG_INFO, "WPS-REG: register timeout failed  ");
			return -1;
		}
	}
	else
		wpa_msg(wpa_s, MSG_INFO, "Setting network authentication: PROTECTED");
#endif /* CONFIG_WPS_ER*/
#endif /* ENABLE_STE_CHANGES*/
	eloop_register_timeout(WPS_PBC_WALK_TIME, 0, wpas_wps_timeout,
			       wpa_s, NULL);

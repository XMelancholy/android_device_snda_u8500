				  ...
				  struct hostapd_config *conf)
{
	...
	/*If the ssid was configured to be WEP enabled,
	then add the wep keys to the bss*/
	for (count = 0; count < NUM_WEP_KEYS; count++) {
		if (ssid->wep_key_len[count]) {
			bss->ssid.wep.len[count] = ssid->wep_key_len[count];
			bss->ssid.wep.key[count] = ssid->wep_key[count];
		}
	}

	if (ssid->wep_tx_keyidx >= 0 &&
			ssid->wep_key_len[ssid->wep_tx_keyidx]) {
		bss->ssid.wep.idx = ssid->wep_tx_keyidx;
		bss->ssid.wep.keys_set = 1;
		wpa_printf(MSG_DEBUG, "WEP default key idx: %d , len: %d\n",
			bss->ssid.wep.idx,
			ssid->wep_key_len[ssid->wep_tx_keyidx]);
	}
#endif
				 ...
				 size_t buflen, int verbose)
{
	...
	ret = os_snprintf(pos, end - pos,
			  ...
			  "group_cipher=%s\n"
			  "key_mgmt=%s\n",
			  wpa_cipher_txt(conf->rsn_pairwise ? conf->rsn_pairwise : conf->wpa_pairwise),

...
static void wpa_supplicant_start_bgscan(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->current_ssid &&
				(wpa_s->current_ssid->bgscan ||
				 wpa_s->conf->default_bgscan ||
				 strlen(DEFAULT_BGSCAN) > 0)) {
...
#ifdef ENABLE_STE_CHANGES
{
	...
	if (wpa_drv_get_country(wpa_s, alpha2)) {
		wpa_printf(MSG_ERROR, "Failed to fetch country code");
	} else {
		wpa_printf(MSG_DEBUG, "Country: %c%c", alpha2[0], alpha2[1]);
	}
}
#endif /* ENABLE_STE_CHANGES */

...
static int hostapd_flush_old_stations(struct hostapd_data *hapd)
{
	...
	/*
	 * Timeout to be sure that the driver will handle broadcast/multicast
	 * frames correctly eg. in case we have stations in PS mode.
	 */
	os_sleep(0, (hapd->conf->dtim_period * hapd->iconf->beacon_int + 20) * 1000);
	if (hostapd_flush(hapd)) {

...
int hostapd_driver_set_p2p_powersave(struct hostapd_data *hapd, int legacy_ps,
		int opp_ps, int ctwindow)
{
	if (hapd->driver && hapd->driver->set_p2p_powersave)
		return hapd->driver->set_p2p_powersave(hapd->drv_priv, legacy_ps,
					     opp_ps, ctwindow);
	return -1;
}

...
#ifdef ENABLE_STE_CHANGES
	wpa_printf(MSG_DEBUG, "p2p mode: %d", hapd->conf->p2p);
/*Add members in P2P group only if running in p2p mode*/
	if (hapd->conf->p2p)
#endif
...
#ifdef ENABLE_STE_CHANGES
	if (beacon_info_changed) {
		wpa_printf(MSG_DEBUG,"beacon_info_changed: %d", beacon_info_changed);
		ieee802_11_set_beacons(hapd->iface);
	}
#endif

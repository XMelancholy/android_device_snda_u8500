					  ...
					  u16 ver)
{
	...
	/* Some IOP Ap misbehaves and send incorrect Key data in EAPOL3 frame.
        To support such APs it is required to discard the return status code
        from wpa_supplicant_parse_ies() */
	if (wpa_supplicant_parse_ies(pos, len, &ie))
		wpa_printf(MSG_INFO, "Incorrect data in unencrypted key data");

...
static void pmksa_cache_expire(void *eloop_ctx, void *timeout_ctx)
{
	...
	os_get_time(&now);
#ifdef ENABLE_STE_CHANGE
	while (pmksa->pmksa && (pmksa->pmksa->caching_date > now.sec
...
#ifdef ENABLE_STE_CHANGES
		{
			if (entry->caching_date > now.sec || entry->expiration <= now.sec) {
				wpa_printf(MSG_DEBUG, "RSN: not valid PMKSA cache entry for "
						MACSTR, MAC2STR(entry->aa));
				pmksa->pmksa = entry->next;
				pmksa_cache_free_entry(pmksa, entry, 0);
				return NULL;

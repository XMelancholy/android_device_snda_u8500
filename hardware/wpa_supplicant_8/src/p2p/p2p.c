...
#ifdef ENABLE_STE_CHANGES
		/* Skip itself */
		else if (os_memcmp(cli->p2p_device_addr, p2p->cfg->dev_addr, ETH_ALEN) == 0) {
				continue;
		}
#endif /* ENABLE_STE_CHANGES */
		else {
...
#ifdef ENABLE_STE_CHANGES
int p2p_update_listen_channel(struct p2p_data *p2p, u8 channel)
{
	if (p2p_channel_to_freq(p2p->cfg->country,
				p2p->cfg->reg_class,
				channel) < 0)
		return -1;

	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Update Listen channel: "
		"channel %u", channel);
	p2p->cfg->channel = channel;

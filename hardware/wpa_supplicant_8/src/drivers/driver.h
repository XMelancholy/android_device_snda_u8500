...
struct wpa_driver_scan_params {
	...
	/**
	 * rssi_filter_value - Used to enable / disable Beacon and Probe Response
	 *                    filtering based on rssi_filter value.
	 *
	 * When 0, filter is disabled, otherwise enabled.
	 */
	int rssi_filter_value;
#endif
};
...
struct ipv4_addr{
	/* Desc: filter out:1 filter in:2*/
	u8 filter_mode;
	/* Desc: source:1 destination:2*/
	u8 addr_mode;
	/* Desc :ip address*/
	u8 addr[4];
};
...
struct set_filter {
	/* 0:IPv4, 1:IPv6, 2:MAC, 3:GROUPMAC, 4:DHCP_Offer,
	5:SSDP, 6:Disable_all*/
	u32 filter_id;

	union filter_data{

		struct ipv4_filter{
			/*Desc: filter in/out all packets on matching IPv4 src address*/
			u8 action_mode; /*0 = disable, 1= enable*/
			/* Desc : No of ip addresses*/
			u8 nrfilter;
			/* Desc : ip address List*/
			struct ipv4_addr ipv4addr[8];
		}ipv4;

		struct ipv6_filter{
			/*Desc: filter in/out all packets on matching IPv6 src address*/
			u8 action_mode; /*0 = disable, 1= enable*/
			/* Desc : No of ip addresses*/
			u8 nrfilter;
			/* Desc : ip address List*/
			struct ipv6_addr ipv6addr[4];
		}ipv6;

		struct mac_filter{
			/*Desc: filter in/out all packets on matching source mac address*/
			u8 action_mode; /* 0=disable, 1=enable*/
			/* Desc : No of Mac addresses*/
			u8 nrfilter;
			/* Desc : Mac address List*/
			struct mac_addr macaddr[8];
		}mac;

		struct groupmac_filter{
			/*Desc: filter in/out all packets on matching group mac address*/
			u8 action_mode; /*0 = disable, 1= enable, 2= dropallmulticast*/
			/* Desc : No of Mac addresses*/
			u8 nrfilter;
			/* Desc : Mac address List*/
			u8 groupmacaddr[8][6];
		...
		struct dhcp_filter{
			...
			/*Desc: filter out all packets on matching protocol : DHCP*/
			u8 action_mode; /*1= enable, 0 = disable*/
		}dhcp;

		struct ssdp_filter{

			/*Desc: filter out all packets on matching protocol : SSDP*/
			u8 action_mode; /*1= enable, 0 = disable*/
		}ssdp;

		struct disableall_filter{

			/*Desc:disable all the packets*/
			u8 action_mode; /*1= enable, 0 = disable*/
		}disableall;
...
#define EVENT_SIGNAL_CHANGE_RSSI       (1<<0)
#define EVENT_SIGNAL_CHANGE_BEACON     (1<<1)
#define EVENT_SIGNAL_CHANGE_TX         (1<<2)
...
struct wpa_driver_ops {
	 ...
	 */
	int (*signal_monitor_rssi)(void *priv, int threshold, int hysteresis);

	/**
	 * signal_monitor_beacon_miss - Set signal monitoring BEACON threshold
	 * @priv: Private driver interface data
	 * @beacon_threshold: Threshold value for signal change events;
	 *	0 = disabled
	 * Returns: 0 on success, -1 on failure (or if not supported)
	 *
	 * This function can be used to configure monitoring of connection
	 * with the current AP. Whenever driver detect beacon loss higher than
	 * %beacon_threshold value, EVENT_SIGNAL_CHANGE event should be
	 * generated.
	 */
	int (*signal_monitor_beacon_miss)(void *priv, int beacon_threshold);

	/**
	 * signal_monitor_tx_fail - Set signal monitoring TX threshold
	 * @priv: Private driver interface data
	 * @tx_threshold: Threshold value for signal change events;
	 *	0 = disabled
	 * Returns: 0 on success, -1 on failure (or if not supported)
	 *
	 * This function can be used to configure monitoring of connection
	 * with the current AP. Whenever driver detect tx frame loss higher
	 * than %beacon_threshold value, EVENT_SIGNAL_CHANGE event should be
	 * generated.
	 */
	int (*signal_monitor_tx_fail)(void *priv, int tx_threshold);
	...
	/**
	 * set_wowlan - set Wake-on-Wireless-LAN (WoWLAN) settings
	 * @priv: Private driver interface data
	 * @flags: WoWLAN flags
	 *
	 * This function set WoWLAN supported settings
	 */
	...
	/**
	* set_data_filter - Sets data filter
	* @data: Data buffer to be sent, set to NULL if no data
	* @data_len: Length of data buffer to be sent
	* After the function returns it contains actual length of reply data
	* buffer.
	*
	* Returns: 0 on success, -1 on failure
	*/
	int (*set_data_filter)(void *priv, const u8 *data, size_t data_len);
#endif
};
...
enum wpa_event_type {
	...
	/**
	 * EVENT_DRIVER_NOTIFY_HANGED - inform about driver hang
	 *
	 * Actions should be taken to reload driver using
	 * WPA_EVENT_DRIVER_STATE "HANGED" towards Android
	 */
	EVENT_DRIVER_NOTIFY_HANGED

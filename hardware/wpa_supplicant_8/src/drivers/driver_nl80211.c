...
static int wpa2_driver_set_p2p_ps(void *priv, int legacy_ps, int opp_ps,
				     int ctwindow);
#endif
...
#ifdef ENABLE_STE_CHANGES
static void nl80211_noa_event(struct wpa_driver_nl80211_data *drv,
			      struct nlattr *tb[])
{
	...
	if (tb[NL80211_ATTR_P2P_PS_NOA_START])
		ed.driver_notify_noa.start_time =
			nla_get_u32(tb[NL80211_ATTR_P2P_PS_NOA_START]);
	if (tb[NL80211_ATTR_P2P_INDEX])
		ed.driver_notify_noa.index =
			nla_get_u8(tb[NL80211_ATTR_P2P_INDEX]);
	if (tb[NL80211_ATTR_P2P_PS_CTWINDOW])
		ed.driver_notify_noa.oppps_ctwindow =
			nla_get_u8(tb[NL80211_ATTR_P2P_PS_CTWINDOW]);
	if (tb[NL80211_ATTR_P2P_PS_OPP_PS])
			if (nla_get_u8(tb[NL80211_ATTR_P2P_PS_OPP_PS]))
				ed.driver_notify_noa.oppps_ctwindow |= BIT(7);
			else
				ed.driver_notify_noa.oppps_ctwindow &= ~BIT(7);
	if (tb[NL80211_ATTR_P2P_PS_NOA_INTERVAL])
		ed.driver_notify_noa.interval =
			nla_get_u32(tb[NL80211_ATTR_P2P_PS_NOA_INTERVAL]);
	if (tb[NL80211_ATTR_P2P_PS_NOA_DURATION])
		ed.driver_notify_noa.duration =
			nla_get_u32(tb[NL80211_ATTR_P2P_PS_NOA_DURATION]);
	if (tb[NL80211_ATTR_P2P_PS_NOA_COUNT])
		ed.driver_notify_noa.count_type =
			nla_get_u8(tb[NL80211_ATTR_P2P_PS_NOA_COUNT]);

	wpa_supplicant_event(drv->ctx, EVENT_DRIVER_NOTIFY_NOA, &ed);
}
	...
	do {
		...
		if (event == NL80211_CQM_RSSI_THRESHOLD_EVENT_HIGH) {
			wpa_printf(MSG_DEBUG, "nl80211: Connection quality monitor "
					      "event: RSSI high");
		ed.signal_change.above_threshold = NL80211_CQM_RSSI_THRESHOLD_EVENT_HIGH;
		} else if (event == NL80211_CQM_RSSI_THRESHOLD_EVENT_LOW) {
			wpa_printf(MSG_DEBUG, "nl80211: Connection quality monitor "
					      "event: RSSI low");
			ed.signal_change.above_threshold = NL80211_CQM_RSSI_THRESHOLD_EVENT_LOW;
		} else
			break;
		ed.signal_change.event_mask = EVENT_SIGNAL_CHANGE_RSSI;
		wpa_supplicant_event(drv->ctx, EVENT_SIGNAL_CHANGE, &ed);
	} while(0);

	/* BEACON LOSS */
	do {
		if (cqm[NL80211_ATTR_CQM_BEACON_MISS_THOLD_EVENT] == NULL)
			break;

		os_memset(&ed, 0, sizeof(ed));
		ed.signal_change.event_mask = EVENT_SIGNAL_CHANGE_BEACON;
		wpa_supplicant_event(drv->ctx, EVENT_SIGNAL_CHANGE, &ed);
	} while(0);

	/* TX LOSS */
	do {
		if (cqm[NL80211_ATTR_CQM_TX_FAIL_THOLD_EVENT] == NULL)
			break;

		os_memset(&ed, 0, sizeof(ed));
		ed.signal_change.event_mask = EVENT_SIGNAL_CHANGE_TX;
		wpa_supplicant_event(drv->ctx, EVENT_SIGNAL_CHANGE, &ed);
	} while(0);
				 ...
				 int cmd, struct nlattr **tb)
{
	...
	case NL80211_CMD_DEL_STATION:
		...
		break;
#ifdef ENABLE_STE_CHANGES
	case NL80211_CMD_NOTIFY_NOA:
		nl80211_noa_event(drv, tb);
		break;
	case NL80211_CMD_NOTIFY_DRIVER_HANG:
		wpa_printf(MSG_DEBUG, "nl80211: Driver hanged notification");
		wpa_supplicant_event(drv->ctx, EVENT_DRIVER_NOTIFY_HANGED,
				     NULL);
		break;
#endif /* ENABLE_STE_CHANGES */
				   ...
				   struct wpa_driver_scan_params *params)
{
	...
	if (params->freqs) {
		for (i = 0; params->freqs[i]; i++) {
			wpa_printf(MSG_DEBUG, "nl80211: Scan frequency %u "
				   "MHz", params->freqs[i]);
	...
	NLA_PUT_U32(msg, NL80211_ATTR_SCAN_RSSI_FILTER, params->rssi_filter_value);
#endif /* CONFIG_RSSI_FILTER */
...
#ifdef ENABLE_STE_CHANGES
	wpa_printf(MSG_DEBUG, "  * uapsd=0x%X", params->uapsd);
	NLA_PUT_U32(msg, NL80211_ATTR_UAPSD, params->uapsd);
#endif /* ENABLE_STE_CHANGES */
...
static int get_sta_handler(struct nl_msg *msg, void *arg)
{
	...
	if (stats[NL80211_STA_INFO_TX_BITRATE]) {
		if (nla_parse_nested(rinfo, NL80211_RATE_INFO_MAX,
			stats[NL80211_STA_INFO_TX_BITRATE], rate_policy)) {
			fprintf(stderr, "failed to parse nested rate attributes!\n");
		} else {
			if (rinfo[NL80211_RATE_INFO_BITRATE]) {
				data->current_tx_rate = nla_get_u16(rinfo[NL80211_RATE_INFO_BITRATE]);
			}
		}
	}
#endif /* ENABLE_STE_CHANGES */
...
static void wpa_driver_nl80211_suspend(void *priv)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	if (linux_set_iface_flags(drv->global->ioctl_sock, bss->ifname, 0)) {
...
static int nl80211_signal_monitor_rssi(void *priv, int threshold, int hysteresis)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
...
static int nl80211_signal_monitor_beacon_miss(void *priv, int beacon_threshold)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg, *cqm = NULL;

	wpa_printf(MSG_DEBUG, "nl80211: Signal monitor beacon_threshold=%d ",
		   beacon_threshold);
	...
	NLA_PUT_U32(cqm, NL80211_ATTR_CQM_BEACON_MISS_THOLD, beacon_threshold);
...
static int nl80211_signal_monitor_tx_fail(void *priv, int tx_threshold)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg, *cqm = NULL;

	wpa_printf(MSG_DEBUG, "nl80211: Signal monitor tx_threshold=%d ",
		   tx_threshold);
	...
	NLA_PUT_U32(cqm, NL80211_ATTR_CQM_TX_FAIL_THOLD, tx_threshold);
...
#elif ENABLE_STE_CHANGES
		wpa2_driver_set_p2p_ps(priv, legacy_ps, opp_ps, ctwindow);
#else
					  ...
					  struct wpa_driver_wowlan_params *wowlan_params)
{
	...
	triggers = nlmsg_alloc();
	if (!triggers) {
		nlmsg_free(msg);
		return -ENOMEM;
	}

	if (wowlan_params->any) {
		NLA_PUT_FLAG(triggers, NL80211_WOWLAN_TRIG_ANY);
	}
	if (wowlan_params->disconnect) {
		NLA_PUT_FLAG(triggers, NL80211_WOWLAN_TRIG_DISCONNECT);
	}
	if (wowlan_params->magic_pkt) {
		NLA_PUT_FLAG(triggers, NL80211_WOWLAN_TRIG_MAGIC_PKT);
	}

	ret = nla_put_nested(msg, NL80211_ATTR_WOWLAN_TRIGGERS, triggers);
	nlmsg_free(triggers);
	if (ret)
		goto nla_put_failure;

	if (wowlan_params->n_patterns) {
		wpa_printf(MSG_DEBUG, "TODO: Patterns, implement me!");
	}

	ret = send_and_recv_msgs(drv, msg, NULL, NULL);
	msg = NULL;
	if (ret)
		wpa_printf(MSG_DEBUG, "nl80211_set_wowlan failed: ret=%d "
			   "(%s)", ret, strerror(-ret));
...
static int get_noa_handler(struct nl_msg *msg, void *arg)
{
	struct nlattr *tb[NL80211_ATTR_MAX + 1];
	struct genlmsghdr *gnlh = nlmsg_data(nlmsg_hdr(msg));
	struct get_noa *noa = (struct get_noa *) arg;

	nla_parse(tb, NL80211_ATTR_MAX, genlmsg_attrdata(gnlh, 0),
	genlmsg_attrlen(gnlh, 0), NULL);


	if (tb[NL80211_ATTR_P2P_PS_NOA]) {
		memcpy(noa->buf, nla_data(tb[NL80211_ATTR_P2P_PS_NOA]),
		       min_int(nla_len(tb[NL80211_ATTR_P2P_PS_NOA]), noa->buf_len));
	}

	return NL_SKIP;
}

static int nl80211_get_noa(void *priv, u8 *buf, size_t buf_len)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
	...
	nl80211_cmd(drv, msg, 0, NL80211_CMD_GET_NOA);
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, drv->ifindex);

	return send_and_recv_msgs(bss->drv, msg, get_noa_handler, &noa);
nla_put_failure:
	nlmsg_free(msg);
	return -ENOBUFS;
}

static int nl80211_set_noa(void *priv, u8 count, int start, int duration, int interval)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
	...
	nl80211_cmd(drv, msg, 0, NL80211_CMD_SET_NOA);
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, drv->ifindex);
	NLA_PUT_U8(msg, NL80211_ATTR_P2P_PS_NOA_COUNT, count);
	NLA_PUT_U32(msg, NL80211_ATTR_P2P_PS_NOA_START, start);
	NLA_PUT_U32(msg, NL80211_ATTR_P2P_PS_NOA_DURATION, duration);
	NLA_PUT_U32(msg, NL80211_ATTR_P2P_PS_NOA_INTERVAL, interval);
				     ...
				     int ctwindow)
{
	...
	nl80211_cmd(drv, msg, 0, NL80211_CMD_SET_P2P_POWER_SAVE);
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, drv->ifindex);
	NLA_PUT_U32(msg, NL80211_ATTR_P2P_PS_LEGACY_PS, legacy_ps);
	NLA_PUT_U32(msg, NL80211_ATTR_P2P_PS_OPP_PS, opp_ps);
	NLA_PUT_U32(msg, NL80211_ATTR_P2P_PS_CTWINDOW, ctwindow);
...
static int nl80211_set_data_filter(void *priv, const u8 *data, size_t data_len)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	...
	nl80211_cmd(drv, msg, 0, NL80211_CMD_SET_DATA_FILTER);
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, drv->ifindex);

	if (data)
		NLA_PUT(msg, NL80211_ATTR_DATA_FILTER, data_len, data);

	wpa_printf(MSG_DEBUG,
			"nl80211: Sending set_data_filter cmd without expecting reply");
	return send_and_recv_msgs(drv, msg, NULL, NULL);
...
#endif /*defined(ENABLE_STE_CHANGES) && defined(CONFIG_DATA_PKT_FILTER)*/
...
const struct wpa_driver_ops wpa_driver_nl80211_ops = {
	...
	.suspend = wpa_driver_nl80211_suspend,
	.send_ft_action = nl80211_send_ft_action,
	.signal_monitor_rssi = nl80211_signal_monitor_rssi,
	.signal_monitor_beacon_miss = nl80211_signal_monitor_beacon_miss,
	.signal_monitor_tx_fail = nl80211_signal_monitor_tx_fail,
	...
	.get_noa = nl80211_get_noa,
	.set_noa = nl80211_set_noa,
#if defined(ENABLE_STE_CHANGES) && defined(CONFIG_DATA_PKT_FILTER)
	.set_data_filter = nl80211_set_data_filter,
#endif

...
#ifdef ENABLE_STE_CHANGES
	/* Always check USIM support. It's required for EAP-SIM on USIM */
	if (sim_type == SCARD_USIM_ONLY || sim_type == SCARD_TRY_BOTH ||
	    sim_type == SCARD_GSM_SIM_ONLY) {
		wpa_printf(MSG_DEBUG, "SCARD: verifying USIM support");
		if (_scard_select_file(scard, SCARD_FILE_UMTS_DF, buf, &blen,
				       SCARD_USIM, NULL, 0)) {
#else
	if (sim_type == SCARD_USIM_ONLY || sim_type == SCARD_TRY_BOTH) {
		wpa_printf(MSG_DEBUG, "SCARD: verifying USIM support");
...
#ifdef ENABLE_STE_CHANGES
		wpa_printf(MSG_DEBUG, "SCARD: Try to read 3G USIM");
		blen = sizeof(buf);

		if (scard_select_file(scard, SCARD_FILE_UMTS_DF, buf, &blen)) {
			wpa_printf(MSG_DEBUG, "SCARD: Failed to read 3G USIM");
			goto failed;
		}
#else
...
#ifdef ENABLE_STE_CHANGES
	/* In case of mobile device, there is no need to check if SIM
	 * is PIN protected. SIM unlocking is done during system startup. */
	scard->pin1_required = 0;
#else
...
#ifdef ENABLE_STE_CHANGES
static int _scard_select_file_mode(struct scard_data *scard, unsigned char *cmd,
			      int cmdlen, unsigned char *buf, size_t *buf_len, unsigned char mode)
{
	long ret;

	cmd[3] = mode;

	ret = scard_transmit(scard, cmd, cmdlen, buf, buf_len);
	...
	if ((buf[*buf_len - 2] != 0x90) && (buf[*buf_len - 1] != 0x00))
	{
		wpa_printf(MSG_WARNING, "SCARD: Select failed "
			"(status=0x%1x%1x)", buf[*buf_len - 2], buf[*buf_len - 1]);
		return -1;
	}

	/* Don't care about status bits */
	buf_len -= 2;
			      ...
			      size_t aidlen)
{
	...
	if (sim_type == SCARD_USIM) {
		...
		if (file_id == SCARD_FILE_UMTS_DF) {
			cmdlen = 7;
			return _scard_select_file_mode(scard, cmd, cmdlen, buf, buf_len, 0x04);
		}


		/* Workarround for reading IMSI file. Complete path needs to be given. */
		if (file_id == SCARD_FILE_GSM_EF_IMSI) {
			cmd[4] = 0x06;
			cmd[5] = SCARD_FILE_MF >> 8;
			cmd[6] = SCARD_FILE_MF & 0xff;
			cmd[7] = SCARD_FILE_UMTS_DF >> 8;
			cmd[8] = SCARD_FILE_UMTS_DF & 0xff;
			cmd[9] = SCARD_FILE_GSM_EF_IMSI >> 8;
			cmd[10] = SCARD_FILE_GSM_EF_IMSI & 0xff;
...
#ifdef ENABLE_STE_CHANGES
		/* If status is 0x9000 for USIM, it's ok and there is nothing to read. */
		wpa_printf(MSG_WARNING, "SCARD: check for 0x9000 status");
		if (sim_type == SCARD_USIM && resp[0] == 0x90 && resp[1] == 0x00) {
			*buf_len = 2;
			buf[0] = 0x90;
			buf[1] = 0x00;
			return 0;
...
#ifdef ENABLE_STE_CHANGES
		wpa_printf(MSG_DEBUG, "SCARD: file length determination. Check status");
		if (scard->sim_type == SCARD_USIM && buf[blen - 2] == 0x90 && buf[blen - 1] == 0x00)
		{
			blen -= 2;
			return blen;
		}
...
{
#ifdef ENABLE_STE_CHANGES
	/* Workarround for 2G on 3G AUTH. Add one byte for max response length. */
...
#ifdef ENABLE_STE_CHANGES
		/* Workarround for 2G on 3G AUTH. Set expected data len to 0xFF otherwise
		 * only status code will be returned and AUTH will fail. */
		cmdlen = 5 + 1 + 16 + 1;
		cmd[cmdlen-1] = 0xFF;
#else
		cmdlen = 5 + 1 + 16;
...
#ifdef ENABLE_STE_CHANGES
	/* Check if two last bytes are different than success status */
	if ((buf[len - 2] != 0x90) && (buf[len - 1] != 0x00))
	{
		wpa_printf(MSG_WARNING, "SCARD: unexpected status from GSM"
			   "auth request (len=%ld status=%02x %02x)",
			   (long) len, buf[len - 2], buf[len - 1]);
		wpa_hexdump(MSG_DEBUG, "SCARD: GSM auth - resp", buf, len);
		return -3;
	}
#else
	if ((scard->sim_type == SCARD_GSM_SIM &&

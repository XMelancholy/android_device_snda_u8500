...
bt_hci_transport_device_type bt_hci_set_transport()
{
    ...
    ret = property_get("ro.ste.bt.hci_transport", transport_type, NULL);
    if(ret == 0)
        printf("ro.ste.bt.hci_transport not set\n");
    else
        printf("ro.ste.bt.hci_transport: %s \n", transport_type);
